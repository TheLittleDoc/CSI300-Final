...
app.put('/courses/:id', (req, res) => {
    const {CoursePrefix, CourseCode, CourseName, Description} = req.body;
    db.run('UPDATE Course SET CoursePrefix = ?, CourseCode = ?, CourseName = ?, Description = ? WHERE CourseID = ?',
...
app.delete('/courses/:id', (req, res) => {
    db.run('DELETE FROM Course WHERE CourseID = ?', [req.params.id], function (err) {
        if (err) return res.status(500).json(err);
        res.json({deleted: this.changes});
    });
});
...
app.post('/sections', (req, res) => {
    ...
    db.run('INSERT INTO Section (CourseID, Weekday, MeetingTime) VALUES (?, ?, ?)',
...
app.put('/sections/:id', (req, res) => {
    const {CourseID, Weekday, MeetingTime} = req.body;
    db.run('UPDATE Section SET CourseID = ?, Weekday = ?, MeetingTime = ? WHERE SectionID = ?',
...
app.delete('/sections/:id', (req, res) => {
    db.run('DELETE FROM Section WHERE SectionID = ?', [req.params.id], function (err) {
        if (err) return res.status(500).json(err);
        res.json({deleted: this.changes});
    });
});
...
app.post('/assignments', (req, res) => {
    ...
    db.run('INSERT INTO Assignment (Name, SectionID, Description, MaterialURL) VALUES (?, ?, ?, ?)',
...
app.put('/assignments/:id', (req, res) => {
    const {Name, SectionID, Description, MaterialURL} = req.body;
    db.run('UPDATE Assignment SET Name = ?, SectionID = ?, Description = ?, MaterialURL = ? WHERE AssignmentID = ?',
...
app.delete('/assignments/:id', (req, res) => {
    db.run('DELETE FROM Assignment WHERE AssignmentID = ?', [req.params.id], function (err) {
        if (err) return res.status(500).json(err);
        res.json({deleted: this.changes});
    });
});
...
app.post('/enrollments', (req, res) => {
    ...
    db.run('INSERT INTO Enrollment (StudentID, SectionID) VALUES (?, ?)',
        [StudentID, SectionID],
        function (err) {
            if (err) return res.status(500).json(err);
            res.json({success: true});
...
app.delete('/enrollments', (req, res) => {
    const {StudentID, SectionID} = req.body;
    db.run('DELETE FROM Enrollment WHERE StudentID = ? AND SectionID = ?', [StudentID, SectionID], function (err) {
        if (err) return res.status(500).json(err);
        res.json({deleted: this.changes});
    });
});
